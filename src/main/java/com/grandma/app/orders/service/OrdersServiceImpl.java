...
import com.grandma.app.orders.mapper.IOrderMapper;
...
public class OrdersServiceImpl implements OrdersService {
        ...
        public OrdersServiceImpl(OrdersRepository ordersRepository, IOrderMapper orderMapper,
        ...
        public OrderDtoResponse createOrder(OrderDto orderDto) {
                var foundClient = clientsRepository.findByDocument(orderDto.getClientDocument())
                                .orElseThrow(() -> new ClientNotFoundException(
                                                String.format("Client con documento %s no encontrado",
                                                                orderDto.getClientDocument())));

                var existsProduct = productsRepository.findByUuid(orderDto.getProductUuid())
                                .orElseThrow(() -> new ProductNotFoundException(
                                                String.format("Producto con uuid %s no encontrado",
                                                                orderDto.getProductUuid())));

                OrderEntity orderEntity = orderMapper.orderDtoToOrderEntity(orderDto);

                orderEntity.setClient(foundClient);
                orderEntity.setProduct(existsProduct);
                ...
                return orderMapper
                                .orderEntityToOrderDtoResponse(ordersRepository.save(orderEntity));
        }

        public OrderDtoResponse updateOrder(String uuid, LocalDateTime timestamp) {
                OrderEntity order = ordersRepository.findById(uuid)
                                .orElseThrow(() -> new OrderNotFoundException(
                                                String.format("Pedido con uuid %s no encontrado", uuid)));

                order.setDelivered(true);
                order.setDeliveredDate(timestamp);

                return orderMapper.orderEntityToOrderDtoResponse(ordersRepository.save(order));
        }
}
